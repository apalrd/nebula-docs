...
and as a `"Key: Value"` format. Queries for nebula certificate info are only returned via the Nebula network, 
so you cannot query this information via the public IP of the lighthouse!
...
If your client names are fully-qualified domain names and under a domain which you control, you can delegate authoritative DNS 
of that zone to the Nebula lighthouse. With this setup, clients can use any domain name server they wish and names will resolve
using normal public DNS servers. However, only nodes within the nebula network will be able to connect to the services, as the 
addresses are still within the private nebula network. 

For example, the domain `nebula.example.net` could be used for the Nebula hosts. The `example.net` zone would add an NS
record `nebula` pointing to the public domain name of the Nebula lighthouse(s). The host `alice-laptop.nebula.example.net` could
then be resolved by any host on the public internet.

If you do not wish to delegate a domain, you can also configure your clients to query the public or nebula
IP of the lighthouse(s), or configure the resolver on your network to forward queries for the nebula domain to the lighthouse.
Please note that the lightouse will not return results for any hostname which is not part of the nebula
network and will not forward queries to other DNS servers, so hosts will still need another DNS server to resolve public 
addresses. Please be are that currently Nebula does not return nxdomain if the query fails, which may confuse the system 
stub resolver on some operating systems. [slackhq/nebula/issues/831](https://github.com/slackhq/nebula/issues/841) is 
tracking this issue.
